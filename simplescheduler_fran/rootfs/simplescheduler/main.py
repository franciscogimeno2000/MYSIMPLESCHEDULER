...
def webserver_home():
    return render_template('index.html',
                           ...
                           groups=get_groups(), is_admin=is_admin_user())


@app.route("/new")
def webserver_new():
    return render_template('new.html', is_admin=is_admin_user())
...
def webserver_edit():
    ...
    return render_template('edit.html',
                           ...
                           weekday=weekday,
                           switchlist=get_switch_html_select_options(),
                           is_new=is_new, is_admin=is_admin_user())
...
def webserver_config():
    return render_template('config.html',
                           o=get_options(),
                           d=valid_domains,
                           notifiers=get_notifiers(), is_admin=is_admin_user())
...
def is_admin_user():
    """UI-only role check (NOT security hardening).

    Requirement: ONLY the Home Assistant user named 'FRAN' is treated as admin.
    Everyone else gets the limited UI.

    We read Home Assistant Ingress headers:
      - X-Remote-User-Name
      - X-Remote-User-Display-Name

    If those headers are missing, we assume NOT admin (safe default).
    """
    uname = (request.headers.get("X-Remote-User-Name", "") or "").strip()
    dname = (request.headers.get("X-Remote-User-Display-Name", "") or "").strip()

    if not uname and not dname:
        return False

    target = "fran"
    if uname.lower() == target:
        return True
    if dname.lower() == target:
        return True
    return False
